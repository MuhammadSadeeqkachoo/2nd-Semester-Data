...
switch(x){
    case 1:
    cout<<"Computer system department";
    break;
...
void enter(){
		...
		int ch=0;
			...
			for(int i=0;i<ch;i++){
				...
				}while(data[i].marks<0||data[i].marks>100);
...
void show(){
	...
	if(total==0){
		...
		for(int i=0;i<total;i++){
                ...
                cout<<"HSSC percentage = "<<data[i].marks<<" %"<<endl;
	    		...
	    		if(data[i].marks>=90&&data[i].marks<=100){
	    		...
	    		else if(data[i].marks>=80&&data[i].marks<90){
	...
	string roll_no;
				...
				for(int i=0;i<total;i++){
					...
					if(roll_no==data[i].rollno){
	    		        ...
	    		        cout<<"Student fee = Rs "<<fee(data[i].Department,data[i].self)<<endl;
